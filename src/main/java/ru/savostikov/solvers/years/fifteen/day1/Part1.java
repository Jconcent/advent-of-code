...
public class Part1 implements PuzzleSolver {
    ...
    public void solvePuzzle() {
        var input = SolverUtility.readInputAsString(Year.TWO_K_FIFTEEN, Day.ONE, PuzzlePart.PART_1).toCharArray();
        ...
        while (startPtr <= endPtr) {
            result += NUMBER_BY_PARENTHESIS.get(input[startPtr]) + (startPtr == endPtr ? 0 : NUMBER_BY_PARENTHESIS.get(input[endPtr]));
            startPtr++;
            endPtr--;
        }

        SolverUtility.printResult(Year.TWO_K_FIFTEEN, Day.ONE, PuzzlePart.PART_1, result);
    }
}
